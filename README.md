...
```bash
git clone https://github.com/andin1st/BPS.git && cd BPS && sudo ./install.sh
```
...
3. **Penyalinan Berkas Aman**: Memindahkan file aplikasi `BPS.exe` dan `appsettings.json` ke direktori lokal pengguna asli (`~/.local/share/bps/`) menggunakan hak akses pengguna biasa, mencegah masalah *permission denied*.
